        ...
        if ($request->isMethod('POST')) {
            if (isset($_POST['g-recaptcha-response'])) {
                ...
                if (!$recaptcha) {
                    ...
                    $flashy->error('Prouve que t es pas un robot !');
...
#[Route('/', name: 'user_index', methods: ['GET'])]
    public function indexJson(): Response
    {
        $users = $this->getDoctrine()
            ...
            ->findAll();
        $jsonContent = json_encode($this->normalizer->normalize($users));
        return new Response($jsonContent, 200, ['Content-Type' => 'application/json']);
    }

    #[Route('/{iduser}/json', name: 'app_user_showjson', methods: ['GET'])]
    public function showJson(User $user): Response
    {
        $jsonContent = json_encode($this->normalizer->normalize($user));
        return new Response($jsonContent, 200, ['Content-Type' => 'application/json']);
    }

    #[Route('/mobile', name: 'user_new', methods: ['POST'])]
    public function newJson(Request $request): Response
    {
    ...
    #[Route('/{id}/editmobile', name: 'user_edit', methods: ['PUT'])]
    public function editJson(Request $request, User $user): Response
    {
        $entityManager = $this->getDoctrine()->getManager();
        $user->setUsername($request->get('username'));
        $user->setEmail($request->get('email'));
    ...
    public function deleteJson(Request $request, User $user): Response
    {
        $entityManager = $this->getDoctrine()->getManager();
